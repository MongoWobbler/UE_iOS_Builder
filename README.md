...
After editing the `slack_webhook_url` variable in /UE_iOS_Builder/slack.sh you can post slack messages in terminal with the shell script! Example below:
...
So, depending on your source control configuration, you may have several steps of git_pull.sh and/or p4_sync.sh. For example, you may need to pull engine source from github **and** pull game source as well. However, your game content could be stored in perforce, so then you would use you p4_sync.sh in that case.

### Perforce
Unfortunately, in MacOS, Jenkins does not get `/usr/local/bin` as part of the path, so we must add it ourselves manually **if** that is where the p4 file lives. That is what the first line is about in the p4_sync.sh, `path_to_p4="/usr/local/bin/"`. 
...
Finally, edit the variable `your_git_repo` in `git_pull.sh` to the path to your git repository.
...
Make sure to edit the following variables in `build.sh` to match your configuration.
...
You'll need to install the [AppCenter command line interface](https://github.com/microsoft/appcenter-cli) (CLI) in order for `upload.sh` to work. To do so, first install [node.js](https://nodejs.org/en/). Node.js and npm usually get installed in `/usr/local/bin` which is fine as long as `/usr/local/bin` is in your $PATH.
...
4. Copy the given token and paste it in the `appcenter_token` variable in `upload.sh`.

Make sure to edit the remaining variables in `upload.sh`  to match your workspace/configuration.
...
---
## Step 4 | Archiving
This step differentiates the .ipa that was created by appending the date and time it was created to the file name and zips up the .ipa to save some storage space. The only variable needed to change in `archive.sh` is the `directory` variable that should be the same as the one in Step 3. 
- `directory` = The directory where the .ipa was placed after building. This is the same directory as `archive_dir` specified in the build step + `/IOS` since Unreal Engine adds the IOS directory.
